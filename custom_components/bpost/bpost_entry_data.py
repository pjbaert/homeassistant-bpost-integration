...
async def async_update_sensors(bpost_api: BpostApi):
    ...
    for active_mail in mail["Active"]:
        ...
        sensor_data["mail_today"]["extra"]["mail_ids"].append(active_mail["tagId"])

    return {"sensor": sensor_data, "binary_sensor": binary_sensor_data, "camera": camera_data}
...
class BpostEntryData:
    ...
    def __init__(self, hass: HomeAssistant, data: dict[str, Any], logger: Logger) -> None:
        super().__init__()
        self.api = BpostApi(email=data["email"], token=data["token"],
        ...
        self.coordinator = DataUpdateCoordinator(hass, logger, name="bpost", update_method=async_update_data,
